...
func (m *tuiModel) View() string {
	...
	topLeft := styleAccent.Render("orbitor") + styleMuted.Render("  cinematic console")
	...
	if strings.TrimSpace(m.missionTitle) != "" {
		topLine += "\n" + styleMuted.Render(" "+m.missionTitle+"  ·  "+defaultString(m.missionSummary, "show every important move"))
	}
	...
	listW := max(20, leftW-4)
	sessionsHeader := styleAccent.Render(" control tower")
	sessionsHeader += styleMuted.Render("  active sessions")
	...
	detailBox := panelStyle.Width(rightW - 2).Height(detailsH).Render(
		clampLines(styleMuted.Render(" spotlight")+"\n"+m.renderDetails(detailContentW), detailsH),
	)

	m.viewport.Height = max(4, feedH-3)

	feedHeader := styleMuted.Render(" live stage")
	...
	if m.viewport.AtBottom() {
		feedHeader += styleAccent.Render("  on-air")
	} else {
		feedHeader += styleMuted.Render("  replay")
	}
	feedBox := panelStyle.Width(rightW - 2).Height(feedH).Render(feedHeader + "\n" + m.viewport.View())

	thinkingLabel := " dark"
	if m.isThinking {
		thinkingLabel = " tracking " + spinnerFrames[m.spinnerFrame]
	}
	thinkingHeader := styleMuted.Render(" backstage") + styleCyan.Render(" "+thinkingLabel)
	...
	if m.activeSessionID != "" {
		...
		hint = "Enter sends  ·  Alt+Enter forks  ·  hold Space dictates  ·  Ctrl+./Ctrl+\\ aborts"
	} else {
		promptPrefix = styleMuted.Render(" ❯ ")
		hint = "Enter connects  ·  hold Space dictates  ·  Tab cycles sessions"
	}
	if m.isThinking {
		hint += "  ·  agent performing"
	}
	inputBox := panelStyle.Width(rightW - 2).Height(inputH).Render(
		styleMuted.Render(" prompt stage") + "\n" + promptPrefix + m.input.View() + "\n" + styleMuted.Render("  "+hint),
	)
...
func (m *tuiModel) renderMissionControl(selectedStyle, activeStyle lipgloss.Style, availH int, listW int) string {
	...
	for i := start; i < end; i++ {
		...
		botLine := trimForLine("  "+stateLabel+"  ·  "+fullPath, max(10, listW))
		...
		if isSelected {
			...
			topLine := prefix + s.ID + "  " + styleCyan.Render(trimForLine(backendModel, max(8, listW/3)))
			titleLine := styleText.Render(trimForLine("  "+titleText, max(10, listW)))
			botLine := styleMuted.Render(trimForLine("  "+stateTag+"  ·  "+fullPath, max(10, listW)))
			if isActive {
