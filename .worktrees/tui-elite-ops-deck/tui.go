...
func (m *tuiModel) View() string {
	...
	selectedStyle := lipgloss.NewStyle().
		...
		Background(colSelBg).
		Bold(true)
	activeStyle := lipgloss.NewStyle().Foreground(colAccent).Bold(true)

	leftW := max(28, m.width*24/100)
	...
	counts := m.sessionStateCounts()
	chip := func(label string, value int, sty lipgloss.Style) string {
		return lipgloss.NewStyle().
			...
			BorderForeground(colBorder).
			Padding(0, 1).
			Render(styleMuted.Render(label+" ") + sty.Render(strconv.Itoa(value)))
	}
	topLeft := styleAccent.Render("orbitor") + styleMuted.Render("  elite ops deck")
	missionLine := styleMuted.Render(" " + defaultString(m.missionTitle, "multi-session command bridge"))
	if strings.TrimSpace(m.missionSummary) != "" {
		missionLine += styleMuted.Render("  ·  " + trimForLine(m.missionSummary, max(20, m.width/2)))
	}
	topRight := lipgloss.JoinHorizontal(
		lipgloss.Top,
		chip("fleet", len(m.sessions), styleAccent),
		chip("live", counts["working"], styleOrange),
		chip("waiting", counts["waiting-input"], styleYellow),
		chip("armed", counts["starting"], styleCyan),
		chip("active", boolToCount(m.activeSessionID != ""), styleGreen),
	)
	topRowGap := m.width - lipgloss.Width(topLeft) - lipgloss.Width(topRight) - 2
	if topRowGap < 1 {
		topRowGap = 1
	}
	topBar := " " + topLeft + strings.Repeat(" ", topRowGap) + topRight + "\n" +
		missionLine + "\n" +
	...
	if pendingSessions := m.pendingPermissionSessions(); len(pendingSessions) > 0 {
		lines := []string{styleYellow.Render(" ⚠ approval queue")}
	...
	statusLeft := fmt.Sprintf("  %s  ·  n=new session  z=zoo  tab=cycle  ctrl+d=delete  ?=help", m.api.baseURL)
	...
	bodyH := max(12, m.height-topBarH-bannerH-statusBarH-6)
	detailsH := 9
	if bodyH < 23 {
		detailsH = 7
	}
	...
	inputH := 4
	feedH := max(7, bodyH-detailsH-thinkingH-inputH)

	sessionsAvailH := max(3, bodyH-1)
	listW := max(24, leftW-4)
	sessionsHeader := styleAccent.Render(" fleet")
	sessionsHeader += styleMuted.Render("  command lanes")
	...
	if m.viewport.AtBottom() {
		feedHeader += styleAccent.Render("  live tail")
	} else {
		feedHeader += styleMuted.Render("  scrollback ready")
	}
	feedBox := panelStyle.Width(rightW - 2).Height(feedH).Render(feedHeader + "\n" + m.viewport.View())

	thinkingLabel := " stable"
	if m.isThinking {
		thinkingLabel = " live analysis " + spinnerFrames[m.spinnerFrame]
	}
	thinkingHeader := styleMuted.Render(" telemetry") + styleCyan.Render("  "+thinkingLabel)
	...
	if m.activeSessionID != "" {
		...
		hint = "Enter dispatches  ·  Alt+Enter forks  ·  hold Space to dictate  ·  Ctrl+./Ctrl+\\ aborts"
	} else {
		promptPrefix = styleMuted.Render(" ❯ ")
		hint = "Enter connects  ·  hold Space to dictate  ·  Tab cycles sessions"
	}
	if m.isThinking {
		hint += "  ·  agent in flight"
	}
	inputBox := panelStyle.Width(rightW - 2).Height(inputH).Render(
		styleMuted.Render(" composer") + "\n" + promptPrefix + m.input.View() + "\n" + styleMuted.Render("  "+hint),
	)
...
func (m *tuiModel) renderDetails(contentWidth int) string {
	...
	chip := func(label, value string, sty lipgloss.Style) string {
		return lipgloss.NewStyle().
			Border(lipgloss.RoundedBorder()).
			BorderForeground(colBorder).
			Padding(0, 1).
			Render(styleMuted.Render(label+" ") + sty.Render(value))
	}
	valW := max(12, contentWidth-4)
	row1 := lipgloss.JoinHorizontal(
		lipgloss.Top,
		chip("state", state, stateStyle(state)),
		chip("backend", s.Backend, styleCyan),
		chip("model", defaultString(s.Model, "default"), styleAccent),
		chip("tool", tool, styleOrange),
	)
	row2 := lipgloss.JoinHorizontal(
		lipgloss.Top,
		chip("pending", boolLabel(s.PendingPermission), styleYellow),
		chip("running", boolLabel(s.IsRunning), styleOrange),
		chip("plan", boolLabel(s.PlanMode), styleViolet),
		chip("skip", boolLabel(s.SkipPermissions), styleRed),
	)
	if s.QueueDepth > 0 {
		row2 = lipgloss.JoinHorizontal(lipgloss.Top, row2, chip("queue", strconv.Itoa(s.QueueDepth), styleYellow))
	}
	lines := []string{
		styleLabel.Render("session") + " " + styleAccent.Render(s.ID),
		row1,
		row2,
		styleLabel.Render("workspace") + " " + styleText.Render(trimForLine(homeTildePath(s.WorkingDir), valW)),
		styleLabel.Render("last") + " " + styleText.Render(trimForLine(defaultString(s.LastMessage, "—"), valW)),
	}
	if s.CurrentPrompt != "" {
		lines = append(lines, styleLabel.Render("task")+" "+styleText.Render(trimForLine(s.CurrentPrompt, valW)))
	}
	if s.Summary != "" {
		lines = append(lines, styleLabel.Render("brief")+" "+styleText.Render(trimForLine(s.Summary, valW)))
	}
...
func (m *tuiModel) renderMissionControl(selectedStyle, activeStyle lipgloss.Style, availH int, listW int) string {
	...
	for i := start; i < end; i++ {
		...
		topLine := trimForLine(prefix+s.ID+"  "+backendModel, max(10, listW))
		midLine := trimForLine("  "+titleText, max(10, listW))
		stateLine := "  " + stateStyle(state).Render(stateLabel)
		if s.QueueDepth > 0 {
			stateLine += styleYellow.Render(fmt.Sprintf("  +%d queued", s.QueueDepth))
		}
		if s.PlanMode {
			stateLine += " " + styleViolet.Render("plan")
		}
		if s.SkipPermissions {
			stateLine += " " + styleOrange.Render("skip")
		}
		botLine := trimForLine("  "+fullPath, max(10, listW))
		sep := styleSep.Render(strings.Repeat("─", listW))
		if isSelected {
			out = append(out, selectedStyle.Render(topLine), selectedStyle.Render(midLine), selectedStyle.Render(trimForLine(stateLine, max(10, listW))), selectedStyle.Render(botLine), sep)
		} else {
			titleLine := styleText.Render(midLine)
			stateTag := trimForLine(stateLine, max(10, listW))
			botLine := styleMuted.Render(trimForLine("  "+fullPath, max(10, listW)))
			if isActive {
				topLine = activeStyle.Render(prefix+s.ID) + "  " + styleCyan.Render(trimForLine(backendModel, max(8, listW/3)))
			}
			out = append(out, trimForLine(topLine, max(10, listW))+badges, titleLine, stateTag, botLine, sep)
		}
	}
...
func (m *tuiModel) sessionStateCounts() map[string]int {
