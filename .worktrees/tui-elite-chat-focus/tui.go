...
func (m *tuiModel) View() string {
	...
	topRight := connLabel + styleMuted.Render("  ·  "+fmt.Sprintf("%d threads", len(m.sessions)))
	topLeft := styleAccent.Render("orbitor") + styleMuted.Render("  chat focus")
	...
	if strings.TrimSpace(m.missionTitle) != "" {
		topLine += "\n" + styleMuted.Render(" "+m.missionTitle+"  ·  "+defaultString(m.missionSummary, "conversation mode"))
	}
	...
	listW := max(18, leftW-4)
	sessionsHeader := styleAccent.Render(" threads")
	sessionsHeader += styleMuted.Render("  hot list")
	...
	detailBox := panelStyle.Width(rightW - 2).Height(detailsH).Render(
		clampLines(styleMuted.Render(" focus")+"\n"+m.renderDetails(detailContentW), detailsH),
	)

	m.viewport.Height = max(5, feedH-2)

	feedHeader := styleMuted.Render(" transcript")
	...
	if m.viewport.AtBottom() {
		feedHeader += styleAccent.Render("  synced")
	} else {
		feedHeader += styleMuted.Render("  scrollback")
	}
	feedBox := panelStyle.Width(rightW - 2).Height(feedH).Render(feedHeader + "\n" + m.viewport.View())

	thinkingLabel := " idle"
	if m.isThinking {
		thinkingLabel = " composing " + spinnerFrames[m.spinnerFrame]
	}
	thinkingHeader := styleMuted.Render(" pulse") + styleCyan.Render(" "+thinkingLabel)
	...
	if m.activeSessionID != "" {
		...
		hint = "Enter sends  ·  Shift+Enter newline  ·  Alt+Enter fork  ·  hold Space dictate"
	} else {
		promptPrefix = styleMuted.Render(" ❯ ")
		hint = "Enter connects  ·  hold Space dictate  ·  Tab cycles threads"
	}
	if m.isThinking {
		hint += "  ·  live response"
	}
	inputBox := panelStyle.Width(rightW - 2).Height(inputH).Render(
		styleMuted.Render(" reply") + "\n" + promptPrefix + m.input.View() + "\n" + styleMuted.Render("  "+hint),
	)
...
func (m *tuiModel) renderDetails(contentWidth int) string {
	...
	pills := []string{
		styleLabel.Render("state ") + stateStr,
		styleLabel.Render("backend ") + backendStr,
		styleLabel.Render("model ") + styleText.Render(defaultString(s.Model, "default")),
		styleLabel.Render("tool ") + toolStr,
	}
	if s.PendingPermission {
		pills = append(pills, styleLabel.Render("pending ")+permStr)
	}
	if s.QueueDepth > 0 {
		pills = append(pills, styleLabel.Render("queue ")+styleYellow.Render(strconv.Itoa(s.QueueDepth)))
	}
	lines := []string{
		lipgloss.JoinHorizontal(lipgloss.Top, pills...),
		styleMuted.Render(trimForLine(homeTildePath(s.WorkingDir), contentWidth)),
		styleText.Render(trimForLine(defaultString(s.LastMessage, defaultString(s.CurrentPrompt, "Ready for the next prompt")), contentWidth)),
	}
...
func (m *tuiModel) renderChatBubble(role, text, ts string, alignRight bool, border lipgloss.Color) string {
	maxBubble := max(28, min(m.chatWidth()-8, m.chatWidth()*4/5))
	body := m.renderRichTextBlock(text, maxBubble-4, false)
	header := styleMuted.Render(role + "  " + ts)
...
func (m *tuiModel) renderMessage(msg WSMessage) {
	...
	if msg.Type == "agent_text" {
		...
		if m.agentBlockIdx >= 0 && m.agentBlockIdx < len(m.logs) {
			...
			m.logs[m.agentBlockIdx] = m.renderChatBubble("assistant", m.agentBlockText, m.agentBlockTime.Format("15:04"), false, colAccent)
		} else {
			m.agentBlockIdx = len(m.logs)
			m.agentBlockTime = time.Now()
			m.agentBlockText = d.Text
			m.logs = append(m.logs, m.renderChatBubble("assistant", d.Text, tsStr, false, colAccent))
			if len(m.logs) > 4000 {
	...
	case "prompt_sent":
		...
		if json.Unmarshal(msg.Data, &d) == nil {
			m.log(m.renderChatBubble("you", d.Text, tsStr, true, colCyan))
			m.isThinking = true
...
func (m *tuiModel) renderMissionControl(selectedStyle, activeStyle lipgloss.Style, availH int, listW int) string {
	...
	for i := start; i < end; i++ {
		...
		topLine := trimForLine(prefix+defaultString(s.Title, shortPath(s.WorkingDir)), max(10, listW))
		midLine := trimForLine("  "+stateLabel+"  ·  "+backendModel, max(10, listW))
		sep := styleSep.Render(strings.Repeat("─", listW))
		if isSelected {
			out = append(out, selectedStyle.Render(topLine), selectedStyle.Render(midLine), sep)
		} else {
			var stateTag string
			switch state {
			case "working":
			...
			topLine := prefix + trimForLine(defaultString(s.Title, shortPath(s.WorkingDir)), max(10, listW))
			titleLine := styleMuted.Render(trimForLine("  "+stateTag+"  ·  "+backendModel, max(10, listW)))
			if isActive {
				topLine = activeStyle.Render(topLine)
			}
			out = append(out, trimForLine(topLine, max(10, listW))+badges, titleLine, sep)
		}
	}
